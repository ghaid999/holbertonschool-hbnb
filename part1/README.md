...
<img src="PackageDiagram.png" width="300" alt="Package Diagram">
...
> The sequence starts with a user registration request. A loop fragment retries the request if it fails. An alt fragment validates the request, returning an error if invalid. If valid, the system saves the data to the database, with a second loop fragment retrying the save operation until it succeeds.
...
> The sequence starts with a place creation request. A loop fragment retries the request if it fails. An alt fragment validates the request, returning an error if invalid. If valid, the system saves the data to the database, with a second loop fragment retrying the save operation until it succeeds.
...
> The sequence starts with a review submission request. A loop fragment retries the request if it fails. An alt fragment validates the request, returning an error if invalid. If valid, the system saves the data to the database, with a second loop fragment retrying the save operation until it succeeds.
